  ...
  <body class="container clearfix">
    ...
    float: left;">
      ...
      <div style="float: right;width: 19%">
        <div style="float: right">
          <h1 style="text-transform: uppercase;text-align: right;">estimate</h1>
          <h3 style="text-transform: uppercase;text-align: right;">#{{$estimate->estimate_no}}</h3>
          {{--<h5 style="text-align: left;padding: 8px 5px;">Balance Due</h5>
          <h5 style="text-align: right;">{{$estimate->total - $estimate->payment->sum(amount)}}</h5>
          <br/>--}}
        </div>
        </div>
      </div>
    ...
    float: left;">
      <div style="float:left;width: 50%;">
        ...
        {{ isset($client->fullname)?$client->fullname:''}}<br/>
        {{isset($client->gst_no)?$client->gst_no:''}}</p>
        <p style="text-align: left">Place of supply: {{$estimate->state}}</p>
      </div> 
          ...
          <table style="width: 80%;border-collapse: collapse;border-spacing: 0;">
            <tr style="text-align: right">
              ...
              <tr style="text-align: right">
                ...
                <td class="date">{{date('d M,Y',strtotime($estimate->expiry_date))}}</td>
              </tr>
            </tr>
          </table>
        </div>
    ...
    float: right;">
        ...
        <div style="float: right">
          ...
          <table>
            ...
            @if($client->tax=='yes')
              @if($estimate->state == $company->state)
              <tr style="text-align: right;">
                <td style=" padding: 8px;">CGST</td>
                <td style=" padding: 8px;">{{(int)($estimate->gst)/2}}</td>
              </tr>
              <tr style="text-align: right;">
              ...
              <tr style="text-align: right;">
                ...
                <td style=" padding: 8px;">{{$estimate->gst}}</td>
              </tr>
              @endif
            @endif
            <tr style="text-align: right;">
              <td style=" padding: 8px;">Discount</td>
              <td style=" padding: 8px;">{{isset($estimate->discount)?$estimate->discount ."%":"--"}}</td>
            ...
            <tr style="text-align: right;">
              ...
              <td style=" padding: 8px;">{{$estimate->total}}</td>
            </tr>
          </table>
