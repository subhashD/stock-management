...
@section('body-content')
  ...
  <div class="card-header bg-info text-white">
    Estimations
     <a href="#payments" data-toggle="modal" id="addDate"><span class="pull-right"><i class="fa fa-plus-circle"></i> New Estimation</span></a>
  </div>
       ...
       <table id="table" class="table table-bordered table-striped">
         ...
         <tbody>
          <?php $count=(@$_GET['page']?(intVal(@$_GET['page'])==1?1:intVal(@$_GET['page'])*10-9):1) ?>
              @foreach($est_clients as $client)
                <tr class="text-center">
                   <td>{{ $count++ }}</td>
                   <td>{{ $client->fullname }}</td>
                   <td>{{ $client->contact_no}}<br> {{ ($client->alt_contact==null)?"":$client->alt_contact}}</td>
                   <td>{{ $client->street.",".$client->pincode.",".$client->city.",".$client->state}}</td>
                   <td>
                    {{ $client->estimate->count()}}                    
                   </td>
                   <td>
                      <a href="/getClientEstimate/{{$client->id}}">
  ...
  <!-- Modal content-->
      <div class="modal-content">
        <div class="modal-header bg-primary">
          ...
          <h4 class="modal-title" id="modalTitle">New Estimate</h4>
        </div>
              ...
              <div class="form-group col-sm-10">
                <select class="form-control" required style="width: 100%" name='client_id' >
                  ...
                  @foreach($clients as $client)
                    <option value="{{$client->id}}">{{$client->fullname.' - '.$client->contact_no}}</option>
                  @endforeach
